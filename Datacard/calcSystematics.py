...
def factoryType(d,s):
  ...
  dataHistUp = "%s_%sUp01sigma_%s"%(r0.nominalDataName[:11],s['name'],r0.nominalDataName[-10:])
  dataHistDown = "%s_%sDown01sigma_%s"%(r0.nominalDataName[:11],s['name'],r0.nominalDataName[-10:])
...
def calcSystYields(_nominalDataName,_inputWS,_systFactoryTypes,skipCOWCorr=True):
  ...
  for s, f in _systFactoryTypes.iteritems():
    if f == "a_h":
      systYields["%s_up"%s] = _inputWS.data("%s_%sUp01sigma_%s"%(_nominalDataName[:11],s,_nominalDataName[-10:])).sumEntries()
      systYields["%s_down"%s] = _inputWS.data("%s_%sDown01sigma_%s"%(_nominalDataName[:11],s,_nominalDataName[-10:])).sumEntries()
