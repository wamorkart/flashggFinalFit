...
void optimize_cats_H4G_1(const int NCATS, TString year, TString mass, Double_t precision, TString path, TString outpath, int Normalization, int minEventsData, double xmin ) {
...
// void optimize_cats_H4G(const int NCAT, TString year) {
  ...
  TString selection_sig = "(weight)*(pho1_pt > 30 && pho2_pt > 18 && pho3_pt > 15 && pho4_pt > 15 && abs(pho1_eta) < 2.5 && abs(pho2_eta) < 2.5 && abs(pho3_eta) < 2.5 && abs(pho4_eta) < 2.5 && (abs(pho1_eta) < 1.4442 || abs(pho1_eta) > 1.566) && (abs(pho2_eta) < 1.4442 || abs(pho2_eta) > 1.566) && (abs(pho3_eta) < 1.4442 || abs(pho3_eta) > 1.566) && (abs(pho4_eta) < 1.4442 || abs(pho4_eta) > 1.566) && pho1_electronveto==1 && pho2_electronveto==1 && pho3_electronveto==1 && pho4_electronveto==1  && tp_mass > 100 && tp_mass < 180 && bdt > -0.9 )";
	TString selection_bg_SR = "(bdt_weight)*(pho1_pt > 30 && pho2_pt > 18 && pho3_pt > 15 && pho4_pt > 15 && abs(pho1_eta) < 2.5 && abs(pho2_eta) < 2.5 && abs(pho3_eta) < 2.5 && abs(pho4_eta) < 2.5 && (abs(pho1_eta) < 1.4442 || abs(pho1_eta) > 1.566) && (abs(pho2_eta) < 1.4442 || abs(pho2_eta) > 1.566) && (abs(pho3_eta) < 1.4442 || abs(pho3_eta) > 1.566) && (abs(pho4_eta) < 1.4442 || abs(pho4_eta) > 1.566) && pho1_electronveto==1 && pho2_electronveto==1 && pho3_electronveto==1 && pho4_electronveto==1  && tp_mass > 100 && tp_mass < 180 && bdt > -0.9)";
  TString selection_bg_CR = "(bdt_weight)*(pho1_pt > 30 && pho2_pt > 18 && pho3_pt > 15 && pho4_pt > 15 && abs(pho1_eta) < 2.5 && abs(pho2_eta) < 2.5 && abs(pho3_eta) < 2.5 && abs(pho4_eta) < 2.5 && (abs(pho1_eta) < 1.4442 || abs(pho1_eta) > 1.566) && (abs(pho2_eta) < 1.4442 || abs(pho2_eta) > 1.566) && (abs(pho3_eta) < 1.4442 || abs(pho3_eta) > 1.566) && (abs(pho4_eta) < 1.4442 || abs(pho4_eta) > 1.566) && pho1_electronveto==1 && pho2_electronveto==1 && pho3_electronveto==1 && pho4_electronveto==1  && tp_mass > 100 && tp_mass < 180 && bdt > -0.9)";
  ...
  TChain *file_s_2018 = new TChain("file_s_2018");
	...
	TFile *file_smooth = TFile::Open(path+"BDT_Histos_smoothing_SmoothSuper_bins190.root");
	// TFile *file_s_smooth =  TFile::Open(path+"Hist_Signal_m0p9_95Bins_smoothing_SmoothSuper.root");
  TH1F* hist_S_smooth;
  hist_S_smooth = (TH1F*)file_smooth->Get("h_bdt_signal_SR");
	//hist_S_smooth->Rebin(2);

	// TFile *file_bkg_smooth =  TFile::Open(path+"Hist_DataMix_m0p9_95Bins_smoothing_SmoothSuper.root");
	TH1F* hist_B_SR_smooth;
	hist_B_SR_smooth = (TH1F*)file_smooth->Get("h_bdt_datamix_SR_weighted_smooth");

	TH1F* hist_B_SB_smooth;
	hist_B_SB_smooth = (TH1F*)file_smooth->Get("h_bdt_datamix_SB_weighted_smooth");

	TH1F* hist_D_smooth;
	hist_D_smooth = (TH1F*)file_smooth->Get("h_bdt_data_SB_smooth");
	cout << hist_D_smooth->Integral() << endl;

	//hist_B_smooth->Rebin(2);
  ...
  file_s->Add(path+s.Format("CatTrain_Standard_M60_Run2_2016/signal_m_60_2016_transform.root/SUSYGluGluToHToAA_AToGG_M_60_TuneCUETP8M1_13TeV_pythia8_13TeV_H4GTag_0"));
  file_s->Add(path+s.Format("CatTrain_Standard_M60_Run2_2017/signal_m_60_2017_transform.root/SUSYGluGluToHToAA_AToGG_M_60_TuneCP5_13TeV_pythia8_13TeV_H4GTag_0"));
  file_s->Add(path+s.Format("CatTrain_Standard_M60_Run2_2018/signal_m_60_2018_transform.root/HAHMHToAA_AToGG_MA_60GeV_TuneCP5_PSweights_13TeV_madgraph_pythia8_13TeV_H4GTag_0"));
 ...
 if (year!="Run2"){
   ...
   cout << "Full run2 categorization" << endl;
   tree_bg->Add(path+s.Format("data_mix_2016_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   tree_bg->Add(path+s.Format("data_mix_2017_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   tree_bg->Add(path+s.Format("data_mix_2018_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));

   tree_bg_2016->Add(path+s.Format("data_mix_2016_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   tree_bg_2017->Add(path+s.Format("data_mix_2017_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   tree_bg_2018->Add(path+s.Format("data_mix_2018_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   // tree_bg->Add(path+s.Format("data_mix_2016_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   // tree_bg->Add(path+s.Format("data_mix_2017_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
   // tree_bg->Add(path+s.Format("data_mix_2018_bdtWeight_kinWeight.root/Data_13TeV_H4GTag_0"));
	 //
 ...
 tree_data->Add(path+s.Format("data_%s.root/tagsDumper/trees/Data_13TeV_H4GTag_0",year.Data()));
  ...
  TH1F *hist_S_2016_tmp = new TH1F("hist_S_2016_tmp","hist_S_2016_tmp",int((xmax-xmin)/precision),xmin,xmax);

  s.Form("%s>>hist_S_2016_tmp",what_to_opt.Data());
  ...
  // Get the scale factor; the data mix needs to be scaled by this factor to match data sideband
	...
	sel.Form("%s",(selection_bg_CR+Mgg_sideband).Data());
  ...
  {
	hist_S_2016_tmp->Scale(35.9);
  hist_S_2016->Scale(35.9);
  hist_S_2017->Scale(41.5);
  hist_S_2018->Scale(54.38);
  }
	cout << "[hist_S_2016->Integral() ] " << hist_S_2016->Integral() << endl;
	cout << "[hist_S_2017->Integral() ] " << hist_S_2017->Integral() << endl;
	cout << "[hist_S_2018->Integral() ] " << hist_S_2018->Integral() << endl;
  ...
  // TH1F *hist_S = (TH1F*)hist_S_2016->Clone("hist_S");
	//
	// cout << "[hist_S->Integral() ] " << hist_S->Integral() << endl;
	TH1F *hist_D_sideband;
  hist_D_sideband = (TH1F*)hist_D_smooth ->Clone("hist_D_sideband");
	//
	// cout << "hist_D_sideband->Integral() " << hist_D_sideband->Integral() << endl;

  TH1F *hist_D_sideband_nonorm = new TH1F("hist_D_sideband_nonorm","hist_D_sideband_nonorm",int((xmax-xmin)/precision),xmin,xmax); //200 bins -- data sideband region
  s.Form("%s>>hist_D_sideband_nonorm",what_to_opt.Data());
  sel.Form("%s",(selection_data+Mgg_sideband).Data());
	tree_data->Draw(s,sel,"goff");

	cout << "hist_D_sideband_nonorm Integral: " << hist_D_sideband_nonorm->Integral() << endl;
  ...
  sel.Form("%s",(selection_bg_SR+Mgg_window).Data());
	tree_bg->Draw(s,sel,"goff");
  ...
  {
   double scale_2016 = 0.0506181;
	 double scale_2017 = 0.049003;
	 double scale_2018 = 0.044659;
    ...
    sel.Form("%s",(selection_bg_SR+Mgg_window).Data());
  	tree_bg_2016->Draw(s,sel,"goff");
		...
		// cout << "2016 scale: " << hist_D_sideband_2016->Integral()/hist_B_2016->Integral() << endl;
    ...
    sel.Form("%s",(selection_bg_SR+Mgg_window).Data());
  	tree_bg_2017->Draw(s,sel,"goff");
		...
		// cout << "2017 scale: " << hist_D_sideband_2017->Integral()/hist_B_2017->Integral() << endl;
    ...
    sel.Form("%s",(selection_bg_SR+Mgg_window).Data());
  	tree_bg_2018->Draw(s,sel,"goff");
		...
		// cout << "2018 scale: " << hist_D_sideband_2018->Integral()/hist_B_2018->Integral() << endl;
  ...
  if (Normalization == 5)
  {
    ...
    // TH1F *hist_B_sideband = (TH1F*)hist_B_sideband_2016->Clone("hist_B_sideband");
	 ...
	 TH1F* hist_B;
	 TH1F* hist_S;
	 TH1F* hist_B_sideband;
	 if (smooth){
		 hist_B = (TH1F*)hist_B_SR_smooth->Clone("hist_B");
		 // hist_S = (TH1F*)hist_S_smooth->Clone("hist_S");
     hist_B_sideband = (TH1F*)hist_B_SB_smooth ->Clone("hist_B_sideband");
     // cout << "hist_B: " << hist_B->Integral() << endl;
		 // cout << "hist_S: " << hist_S->Integral() << endl;
		 // cout << "hist_B_sideband: " << hist_B_sideband->Integral() << endl;

	 }
	 else{
		 hist_B = (TH1F*)hist_B_2016->Clone("hist_B");
		 // hist_S = (TH1F*)hist_S_2016->Clone("hist_S");
		 hist_B_sideband = (TH1F*)hist_B_sideband_2016->Clone("hist_B_sideband");
	 }
   // TH1F *hist_B = (TH1F*)hist_B_2016->Clone("hist_B");
   // TH1F *hist_B_sideband = (TH1F*)hist_B_sideband_2016->Clone("hist_B_sideband");
	 // hist_B_sideband->Rebin(2);

	 // hist_D_sideband->Rebin(2);
  hist_S = (TH1F*)hist_S_2016->Clone("hist_S");
	 cout << "hist_B_SR: " << hist_B->Integral() << endl;
	 cout << "hist_S: " << hist_S->Integral() << endl;
	 cout << "hist_B_sideband: " << hist_B_sideband->Integral() << endl;
	 cout << "hist_D_sideband: " << hist_D_sideband->Integral() << endl;
	...
	hist_S2->Scale();
        // cout << "CHECK BKG INTEGRAL: " << hist_B2->Integral() << endl;
        // cout << "Bin content: " << hist_B2->GetBinContent(hist_B2->FindBin(0.98)) << "  "  << hist_B2->GetBinContent(hist_B2->FindBin(1)) << endl;
        // cout << "BKG BIN WIDTH: " << hist_B2->GetBinWidth(189) << "  " << hist_B2->GetBinWidth(190) << endl;
        // cout << "hist_B2->Integral(hist_B2->FindBin(0.98),hist_B2->FindBin(1))):  " << hist_B2->Integral(hist_B2->FindBin(0.98),hist_B2->FindBin(1)) << endl;
        //cout << hist_B2->GetBinLowEdge(189) << "   "  << hist_B2->GetBinLowEdge(190) << endl;
// CMS info
...
std::vector<double> significance_scans3;
               ...
               if (NCATS<=4) {
                  ...
                  bkg_sideband_n[4] = 1.;
					...
					for (int index=0;index<NCATS;index++){ //start from 1 for tth  only when optimizing separately
						...
						// minevt_cond_n[index] = (bkg_sideband_n[index]>=minevents && (data_sideband_n[index] >= minEventsData));
					}
  ...
  cout << "[ " << s.Format("%s%s_%s_xmin-%s_binWidth-%s_CatSignificances.txt",outpath.Data(),outname.Data(),scaleOpt.Data(),xmin_str.Data(),binWidth_str.Data()) << "]" << endl;
	...
	double S_2016, S_2017, S_2018 = 0;
	double S_2016_Norm, S_2017_Norm, S_2018_Norm = 0; // signal integral values normalized to lumi
	double data_sideband_total = 0;
        double bkg_sideband_total = 0;
	double data_sideband_nonorm_total = 0;
	int numBins = int((xmax-xmin)/bin_width);
	//double tot_sig_p1Bin, tot_sig_p2Bin, tot_sig_m1Bin, tot_sig_m2Bin = 0;
        double tot_sig = 0;
        double tot_sig_p1Bin = 0;
        double tot_sig_p2Bin = 0;
        double tot_sig_m1Bin = 0;
        double tot_sig_m2Bin = 0;
	catSigOut << "Category boundary: " <<   "Significance:  "  << "Signal events:  "  << "S_2016 / 35.9 :  "  << "S_2017 / 41.5:  "  << "S_2018 / 54.38:  "  << "Bkg events(SR):  "  <<  "Bkg events(CR):  "  << "Data sideband:  " << "Data sideband (no norm): "  << "\n";
	...
	catSigOut_m2Bin << "Category boundary: " <<   "Significance:  "  << "Signal events:  "  << "Bkg events"  << "\n";
        ...
        cout << tot_sig_p1Bin << "  " << tot_sig_p2Bin << "  "  <<tot_sig_m1Bin << "  "  << tot_sig_m2Bin << endl;
	for(int i = 0; i < NCATS; i++){
		...
		B_total = hist_B2->Integral(min_bin, max_bin);
                cout << "Signal N bins: " << hist_S2->GetNbinsX()  <<  " "  << "Bkg N bins: " << hist_B2->GetNbinsX()  << endl;
 		S_2016 = hist_S_2016_tmp->Integral(min_bin, max_bin);
		S_2017 = hist_S_2017->Integral(min_bin, max_bin);
		S_2018 = hist_S_2018->Integral(min_bin, max_bin);
		S_2016_Norm = S_2016/35.9;
		S_2017_Norm = S_2017/41.5;
		S_2018_Norm = S_2018/54.38;
                cout << "min_bin : " << min_bin << "  max_bin: " << max_bin << endl;
		data_sideband_total = hist_D_sideband->Integral(min_bin, max_bin);
                bkg_sideband_total = hist_B_sideband->Integral(min_bin, max_bin);
		data_sideband_nonorm_total = hist_D_sideband_nonorm->Integral(min_bin, max_bin);
		...
		cout << "S: " << S_total << endl;
		cout << "B (SR): " << B_total << endl;
                cout << "B (CR): " << bkg_sideband_total << endl;
		//cout << "S 2016: " << hist_S_2016_tmp->Integral(min_bin, max_bin) << endl;
		//cout << "S 2017: " << hist_S_2017->Integral(min_bin, max_bin) << endl;
		//cout << "S 2018: " << hist_S_2018->Integral(min_bin, max_bin) << endl;

		//cout << "TOTAL S " << S_2016+S_2017+S_2018 << endl;
                //cout << "B Total " << B_total << endl;
		...
		catSigOut << "Cat: [" << cat_min << ", " << cat_max << "]: " << Cat_significance << " :" <<  S_total << " :" <<  S_2016_Norm << " :"  <<  S_2017_Norm << " :"  <<   S_2018_Norm << " :" <<  B_total << " :" << "  :" << bkg_sideband_total << "  :" << data_sideband_total <<  "  :" << data_sideband_nonorm_total << "\n";
 ...
 cout << "[Total Significance]: " << sqrt(tot_sig) << endl;
