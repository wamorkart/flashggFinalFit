    ...
    // read each line
      ...
      // round up scale syst if less than 1.e-4
        ...
        if (sqrts_==8 || sqrts_==7) catname=Form("cat%d",diphotonCat); // these bits can probably be factorised to avoid repetition
	      if (sqrts_ ==13){
					std::cout << "[FinalModelConstruction.cc] - flashggCats_.size():" << flashggCats_.size() << std::endl;
					std::cout << "[FinalModelConstruction.cc] - flashggCats_[0]:" << flashggCats_[0] << std::endl;
					std::cout << "[FinalModelConstruction.cc] - diphotonCat:" << diphotonCat << std::endl;
					std::cout << "[FinalModelConstruction.cc] - flashggCats_[" << diphotonCat << "]:" << flashggCats_[diphotonCat] << std::endl;
					catname = Form("%s",flashggCats_[diphotonCat].c_str()); // these bits can probably be factorised to avoid repetition
				}
