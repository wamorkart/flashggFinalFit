...
unsigned int getIndexOfReferenceDataset(string proc, string cat){
  ...
  cout << "[IN SIGNAL FIT]: proc = " << proc << endl;
  if (iLine==-1 ) {
    ...
    //loop over it
      ...
      string proc = els[0];
		  ...
		  if( els.size()==6 ) {
            ...
            if (analysis_ == "H4G")
						{
							map_replacement_proc_RV_.push_back( "H4G" );
	        		map_replacement_cat_RV_.push_back( "Cat0" );
	       			map_replacement_proc_WV_.push_back( "H4G" );
	        		map_replacement_cat_WV_.push_back( "Cat0" );
						}
						else{
      ...
      // if (verbose_) std::cout << "[INFO] dat file listing: ----> selected replacements (WV) if needed " <<  map_replacement_proc_WV_[map_replacement_proc_WV_.size() -1] << " " <<  map_replacement_cat_WV_[map_replacement_cat_WV_.size() -1] << std::endl;
			std::cout << "[INFO] dat file listing: "<< proc << " " << cat << " " << nGaussiansRV << " " << nGaussiansWV <<  " " << std::endl;
      std::cout << "[INFO] dat file listing: ----> selected replacements (RV) if needed " <<  map_replacement_proc_RV_[map_replacement_proc_RV_.size() -1] << " " <<  map_replacement_cat_RV_[map_replacement_cat_RV_.size() -1] << std::endl;
      std::cout << "[INFO] dat file listing: ----> selected replacements (WV) if needed " <<  map_replacement_proc_WV_[map_replacement_proc_WV_.size() -1] << " " <<  map_replacement_cat_WV_[map_replacement_cat_WV_.size() -1] << std::endl;
	...
	//plotBeamSpotDZdist(data,"VHdebug");
        ...
        if(analysis_ == "HHWWgg"){
          ...
          data0 = data00;
					...
					RooDataSet *data00;
				  data00 = reduceDataset((RooDataSet*)inWS->data(Form("%sTag_%s_13TeV_%d_%d",proc.c_str(),cat.c_str(),year_,mh)));
				}
        else{
          RooDataSet *data00   = reduceDataset((RooDataSet*)inWS->data(Form("%d%s",mh,proc.c_str()), Form("%s_%d_13TeV_%s",proc.c_str(),mh,cat.c_str())));
          data0 = data00;
        ...
        if (analysis_ == "H4G"){
					...
					data0Ref   = beamSpotReweigh(
                        		...
                        		intLumiReweigh(
                          		reduceDataset(
                          			(RooDataSet*)inWS->data(Form("%sTag_%s_13TeV_%d_%d",proc.c_str(),cat.c_str(),year_,mh))
                              )
                            ), "RV"
                          )
											 );
        ...
        if(analysis_type_ == "NMSSM"){
         ...
         RooDataSet * data0Ref00   = rvwvDataset(
                        intLumiReweigh(
                          reduceDataset(
														(RooDataSet*)inWS->data(Form("%sTag_%s_13TeV_%d_%d",replancementProc.c_str(),replancementCat.c_str(),year_,mh))
														// (RooDataSet*)inWS->data(Form("%d%s",mh,replancementProc.c_str()), Form("%sTag_%s_13TeV_%d",replancementProc.c_str(),replancementCat.c_str(),year_))
