	...
	for (int cat=0; cat<nCats_; cat++){
		for (vector<string>::iterator proc=procs_.begin(); proc!=procs_.end(); proc++){
			...
			if (analysis_ == "HHWWgg"){
				...
				cout << "looking for WS: " << Form("%s_%s",proc->c_str(),flashggCats_[cat].c_str()) << endl;
				RooWorkspace* theWS_ = inWS_->getSpecificWorkspace(0);
				theWS = theWS_;
			}
			...
			if (photonCatScalesStr_.size()!=0){
				for (vector<string>::iterator phoCat=photonCatScales_.begin(); phoCat!=photonCatScales_.end(); phoCat++){
					...
					if (isFlashgg_){
						...
						if (analysis_ == "H4G") {
							// cout << "H4G analysis " << endl;
							// cout << phoCat->c_str() << endl;
							hists = getHistograms(theWS, Form("%s_%s",proc->c_str(),flashggCat.c_str()), Form("MCScale%s",phoCat->c_str()) );
						}
				...
				for (vector<string>::iterator phoCat=photonCatSmears_.begin(); phoCat!=photonCatSmears_.end(); phoCat++){
					...
					if (isFlashgg_){ // Smearing not yet supported for Flashgg
						...
						if (analysis_ == "HHWWgg") {
							...
							hists = getHistograms(theWS, Form("%s_%s",proc->c_str(),flashggCat.c_str()), Form("MCSmear%s",phoCat->c_str()) );
 						}
				...
				if (photonCatScalesCorrStr_.size()!=0){
					for (vector<string>::iterator phoCat=photonCatScalesCorr_.begin(); phoCat!=photonCatScalesCorr_.end(); phoCat++){
						...
						if (analysis_ == "HHWWgg") {
							...
							hists = getHistograms(theWS, Form("%s_%s",proc->c_str(),flashggCat.c_str()), Form("%s",phoCat->c_str()) );
 						}
