...
void OptionParser(int argc, char *argv[]){
    ...
    "If you wish to only consider a subset cat in the list, list them as separated by commas. ")
		("analysis",	po::value<string>(&analysis_)->default_value(""),  	"analysis. Ex: HHWWgg, H4G ")
		("analysis_type",	po::value<string>(&analysis_type_)->default_value(""),  	"analysis type. Ex: Res, EFT, NMSSM")
		("FinalState_",	po::value<string>(&FinalState_)->default_value(""),  	"HHWWgg final state. Ex: qqlnu, lnulnu, qqqq OR H4G final state. Ex. m60, m45 etc")
...
void plot(string outPath, int mh, RooRealVar *var, RooAbsData *data, RooAbsPdf *pdf){
	...
	RooPlot *plot = var->frame(Range(mh-20,mh+20));
			...
			if (isFlashgg_){
				...
				if(analysis_ == "HHWWgg"){
					...
					if(analysis_type_ == "NMSSM"){
						data00   = (RooDataSet*)inWS->data(Form("NMSSM_%s_13TeV_%s",HHWWgg_Label.c_str(),flashggCats_[cat].c_str())); // HHWWgg NMSSM form
					}

					else{
						data00   = (RooDataSet*)inWS->data(Form("%s_%s_13TeV_%s",proc.c_str(),HHWWgg_Label.c_str(),flashggCats_[cat].c_str())); // HHWWgg
					}
					...
					if(verbose_) {
						// cout << "[H4G_Label]" << H4G_Label.c_str() << endl;
						// H4G_Cat0_H4G_Training_CombinedMass_PerYear
						cout << year_.c_str() << endl;
						cout << "dataset to look for: " << Form("%sTag_%s_13TeV_%s",proc.c_str(),flashggCats_[cat].c_str(),year_.c_str()) << endl;
					}
					RooDataSet *data00;
					data00   = (RooDataSet*)inWS->data(Form("%sTag_%s_13TeV_%s",proc.c_str(),flashggCats_[cat].c_str(),year_.c_str())); // eg: H4GTag_Cat0_13TeV
					data0 = data00;
				}
      ...
      // which nGaussians do we want to choose?
        ...
        //do the fit
	        ...
	        //   SumW2Error(true),Verbose(false),Range(mass_-10,mass_+10));
					RooFitResult *fitRes = pdf->fitTo(*dataRV,Save(true),
						RooFit::Minimizer("Minuit","minimize"),
						SumW2Error(true),Verbose(false),Range(rangeLow,rangeHigh));
  ...
  // start with the RV plots
    ...
    //only make plots we asked for...
      ...
      // save it !
        ...
        outdir_.c_str(),proc.c_str(),
        flashggCats_[cat].c_str()));
		canv->Print(Form("%s/rv_%s_%s.png",website.c_str(),proc.c_str(),
        flashggCats_[cat].c_str()));
		canv->Print(Form("%s/rv_%s_%s.pdf",website.c_str(),proc.c_str(),
        flashggCats_[cat].c_str()));
		}
	}
  ...
  // now plot the WV
    ...
    plotIt++){
          ...
          //save plots
            ...
            outdir_.c_str(),proc.c_str(),flashggCats_[cat].c_str()));
			canv->Print(Form("%s/wv_%s_%s.png",website.c_str(),proc.c_str(),
        	flashggCats_[cat].c_str()));
			canv->Print(Form("%s/wv_%s_%s.pdf",website.c_str(),proc.c_str(),
        	flashggCats_[cat].c_str()));
		 }
	}
...
int main(int argc, char *argv[]){
	...
	string website = Form("/eos/user/t/twamorka/www/%s",analysis_type_.c_str());
